...
export async function GET(
    ...
    try {
        ...
        // For movie structure
        $('div.download > div.anime-box, div.download > div.yondarkness-box').each((_, boxElement) => {
            const data: DownloadData[] = [];
            $(boxElement)
                .find('.anime-title, .yondarkness-title')
                .each((__, titleElement) => {
                    const titleText = $(titleElement).text().trim();
                    const qualityMatch = titleText.match(/\[(\d+p)\]/);
                    const sizeMatch = titleText.match(/\[(\d+(?:[.,]\d+)? [Mm][Bb])\]/);
                    const quality = qualityMatch ? qualityMatch[1] : 'Unknown';
                    const size = sizeMatch ? sizeMatch[1] : 'Unknown';
                    const source: Source[] = [];
                    $(titleElement)
                        .next('.anime-item, .yondarkness-item')
                        .find('a')
                        .each((___, link) => {
                            const name = $(link).text().trim();
                            const url = $(link).attr('href') || '';
